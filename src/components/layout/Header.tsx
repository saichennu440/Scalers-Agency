...
interface HeaderProps {
  ...
  onClientsCategory: (category: string) => void; // fires when a dropdown option is picked
}
...
export default function Header({ currentPage, onNavigate, onClientsCategory }: HeaderProps) {
  ...
  /* ── fetch categories whenever content changes ── */
  useEffect(() => {
    const load = async () => {
      ...
      if (data) {
        const unique = Array.from(
          new Set(data.map((d) => (d.category as string).trim()).filter(Boolean))
        ).sort();
    ...
    // realtime: refresh when client_content changes
    const ch = supabase
      .channel('header-categories')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'client_content' }, load)
      .subscribe();
  ...
  const navBg  = scrolled ? 'rgba(6,16,31,0.92)' : isHome ? 'transparent' : 'transparent';
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <button onClick={() => goPage('home')} className="flex items-center gap-2.5 group">
            ...
            <div className="flex flex-col leading-none">
              ...
              <span className="text-[8px] tracking-[0.3em] font-light text-white/40">BUSINESS AGENCY</span>
            </div>
          </button>
          ...
          <nav className="hidden md:flex items-center">
            <div className="flex items-center gap-1 px-2 py-1.5 rounded-xl bg-white/5 border border-white/10">
              ...
              {/* Clients — with category dropdown */}
              <div ref={dropRef} className="relative">
                ...
                {dropOpen && (
                  <div
                    className="absolute top-full left-0 mt-2 min-w-[200px] rounded-2xl overflow-hidden shadow-2xl border border-white/8 z-50"
                  ...
                  >
                    ...
                    {categories.length === 0 && (
                      <div className="px-5 py-4 text-[11px] text-white/20 italic">
                        No categories yet — add content in admin.
                      </div>
                    )}
                  </div>
                )}
