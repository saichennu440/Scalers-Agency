...
export default function ContentModal({ content, onClose }: ContentModalProps) {
  // normalize type to lowercase to avoid caps/extra spaces errors
  const type = (content?.content_type || '').toString().trim().toLowerCase();

  const imageTypes = ['image', 'creatives'];
  const videoTypes = ['video', 'videos', 'reels'];

  const isImage = imageTypes.includes(type);
  const isVideo = videoTypes.includes(type);

  // unified embed detection (not used to decide image vs video)
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="relative bg-[#E8E4D9] w-full sm:max-w-3xl rounded-t-3xl sm:rounded-3xl overflow-hidden shadow-2xl h-[92vh] flex flex-col"
        onClick={(e) => e.stopPropagation()} // prevent backdrop close when clicking inside
        key={content.id ?? 'content-modal'}
      >
        {/* Close */}
        <button
          onClick={onClose}
        ...
        {/* ── Unified Media Block (only one block renders) ── */}
        {(() => {
          // IMAGES
          if (isImage && content.content_url) {
            return (
              <div className="flex-none">
                <div className="aspect-video bg-[#f5f0eb] shrink-0">
                  <img
                    ...
                    onError={(e) => {
                      // optional: hide broken image gracefully (prevents white flash)
          ...
          if (isVideo && content.content_url) {
            return (
              <div className="flex-none">
